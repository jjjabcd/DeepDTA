...
# ------------------ Tokenizers (same as train_from_csv.py) ------------------
...
def build_deepdta_like_model(smiles_len=150, fasta_len=1000,
    ...
    x = layers.Embedding(smiles_vocab_size, emb_dim, mask_zero=False)(inp_smi)
    ...
    y = layers.Embedding(fasta_vocab_size, emb_dim, mask_zero=False)(inp_fa)
    ...
    # compile not strictly necessary for predict, but harmless
...
def main():
    ...
    parser.add_argument("--fasta", type=str, required=True, help="Protein FASTA sequence (letters only)")
    parser.add_argument("--weights", type=str, required=True, help="Path to model weights (e.g., outputs/best.h5)")
    parser.add_argument("--smiles_max_len", type=int, default=150)
    parser.add_argument("--fasta_max_len", type=int, default=1000)
    parser.add_argument("--out_file", type=str, default="", help="Optional CSV to save result with predicted_value column")
